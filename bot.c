        ...
        for (int i = 0; i < SIGHT && seen == 0; ++i)
        {
            for (int j = 0; j < SIGHT && seen == 0; ++j)
            {
                if (player_data->player_minimap[i][j] == '*')
                {
                    ...
                    if ((i - 1) >= 0 && player_data->player_minimap[i-1][j] != '|') possible_action[0] = 1;
                    if ((j + 1) < COLUMNS && player_data->player_minimap[i][j+1] != '|') possible_action[1] = 1;
                    if ((i + 1) < ROWS && player_data->player_minimap[i+1][j] != '|') possible_action[2] = 1;
                    if ((j - 1) >= 0 && player_data->player_minimap[i][j-1] != '|') possible_action[3] = 1;
